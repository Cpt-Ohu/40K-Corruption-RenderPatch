...
namespace _40KPawnRenderer
{
  ...
  //              graphics.headGraphic = ChildGraphics.GetChildHeadGraphics(ShaderDatabase.CutoutSkin, graphics.pawn.story.SkinColor);
        ...
        public static void _RenderPawnInternal(this PawnRenderer _this, Vector3 rootLoc, Quaternion quat, bool renderBody, Rot4 bodyFacing, Rot4 headFacing, RotDrawMode bodyDrawType = 0, bool portrait = false)
        {
            ...
            Mesh mesh = null;
            Pawn pawn = (Pawn)PawnRendererModded.pawnInfo.GetValue(_this);
            ...
            List<HediffComp_DrawImplant> implantDrawers = PawnRendererModded.implantDrawers(pawn);
        ...
        //            Log.Message("drawChaos");
                ...
                if (renderBody || (pawn.InBed() && pawn.ageTracker.CurLifeStageIndex <= 1))
            {
                ...
                {
                    _this.graphics.dessicatedGraphic.Draw(vector, bodyFacing, pawn);
                }
                else
                {
                    ...
                    if (bodyDrawType == 0)
                    {
                        ...
                        vector2.y += 0.02f;
                        ((PawnWoundDrawer)PawnRendererModded.woundInfo.GetValue(_this)).RenderOverBody(vector2, mesh, quat, portrait);
                    }
                }
            }
            ...
            if (_this.graphics.headGraphic != null && pawn.ageTracker.CurLifeStageIndex >= 1)
            {
                Vector3 vector5 = quat * _this.BaseHeadOffsetAt(headFacing);
                Mesh mesh2 = ((pawn is AlienPawn) ? (pawn as AlienPawn).headSet : MeshPool.humanlikeHeadSet).MeshAt(headFacing);
                Material material = _this.graphics.HeadMatAt(headFacing, bodyDrawType);
                ...
                if (pawn.ageTracker.CurLifeStageIndex >= 2)
                {
                        Mesh mesh3 = PawnRendererModded.HairMeshSet(pawn).MeshAt(headFacing);
                ...
                {
                    Mesh mesh4 = PawnRendererModded.HairMeshSet(pawn).MeshAt(headFacing);
                    Material material3 = _this.graphics.HairMatAt(headFacing);
            ...
            if (renderBody)
            {
                for (int k = 0; k < _this.graphics.apparelGraphics.Count; k++)
                {
                    ...
                    if (apparelGraphicRecord.sourceApparel.def.apparel.LastLayer == ApparelLayer.Shell)
                    {
                        ...
                        material4 = _this.graphics.flasher.GetDamagedMat(material4);
                        if (pawn.ageTracker.CurLifeStageIndex == 2)
                        {
                    ...
                    if (CompPauldronDrawer.ShouldDrawPauldron(pawn, _this.graphics.apparelGraphics[k].sourceApparel, bodyFacing, out pauldronMat))
                    {
                        if (pawn.ageTracker.CurLifeStageIndex == 2)
                        {
            ...
            {
                Graphics.DrawMesh(mesh, vector3, quat, _this.graphics.packGraphic.MatAt(pawn.Rotation, null), 0);
            }
            if (!portrait)
            {
                PawnRendererModded.equipInfo.Invoke(_this, new object[]
                {
                    rootLoc
                });
                ...
                ((PawnHeadOverlays)PawnRendererModded.statusInfo.GetValue(_this)).RenderStatusOverlays(vector7, quat, ((pawn is AlienPawn) ? (pawn as AlienPawn).headSet : MeshPool.humanlikeHeadSet).MeshAt(headFacing));
            }
        }
